...
export default function Hashtags({ isKeyword }) {
  ...
  const callGetHashtags = async () => {
    ...
    hashtags = hashtags?.data?.filter(
      (hashtag) => hashtag.isKeyword === isKeyword
    );
  ...
  const handleDelete = async() => {
      ...
      await deleteHashtag(Hashtag, accessToken);
      callGetHashtags();
      setHashtag("");
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        ...
        <DeleteDialog type={isKeyword? "keyword":"hashtag"} open={openDialog} setOpen={setOpenDialog} postAction={handleDelete}/>
      </Grid>
      <Grid item xs={12}>
